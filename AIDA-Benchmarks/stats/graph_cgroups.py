...
def formDataFrame(filename):
    ...
    start_us = df['Start Time (microseconds)'].min()
    end_us = df['Start Time (microseconds)'].max()
...
df_off, t_off = formDataFrame("results/ycsb_gpu_max_0.csv")
df_quart, t_quart = formDataFrame("results/ycsb_gpu_max_4.csv")
df_mid, t_mid = formDataFrame("results/ycsb_gpu_max_6.csv")
df_max, t_max = formDataFrame("results/ycsb_gpu_max_16.csv")

keyword_off = "barebones"
keyword_max = "w/ Linear Regression at 1600% max"
keyword_mid = "w/ Linear Regression at 600% max"
keyword_quart = "w/ Linear Regression at 400% max"
...
plt.plot(df_mid['timestamp'], df_mid[key], linestyle=':', linewidth=2, label=f"{subject} ({keyword_mid})", color='orange')

plt.plot(df_mid['timestamp'], df_quart[key], linestyle=':', linewidth=2, label=f"{subject} ({keyword_quart})", color='green')
...
data = [
    ...
    [f"{keyword_mid}", f"{df_mid[key].mean():.2f}", f"{df_mid[key].quantile(0.95):.2f}", t_mid],
    [f"{keyword_quart}", f"{df_quart[key].mean():.2f}", f"{df_quart[key].quantile(0.95):.2f}", t_quart],
...
plt.savefig(f"cgroup/{bench}{subject}_latency.pdf", format="pdf")
plt.show()