...
df_off = formDataFrame("ycsb_dbml_off.csv")
df_gpu = formDataFrame("ycsb_dbml_gpu2.csv") # gpu
df_cpu = formDataFrame("ycsb_dbml_cpu.csv") #cpu

keyword_off = "barebones"
keyword_gpu = "w/ Torch Linear Regression on GPU"
keyword_cpu = "w/ Torch Linear Regression on CPU"

fig, ax1 = plt.subplots(figsize=(16, 9))

# Plot latency time series
plt.plot(df_gpu['timestamp'], df_gpu[key], linewidth=2, label=f"{subject} ({keyword_gpu})", color='red')

plt.plot(df_cpu['timestamp'], df_cpu[key], linestyle=':', linewidth=2, label=f"{subject} ({keyword_cpu})", color='orange')
...
data = [
    [f"{keyword_gpu}", f"{df_gpu[key].mean():.2f}", f"{df_gpu[key].quantile(0.95):.2f}", f"5736"],
    [f"{keyword_cpu}", f"{df_cpu[key].mean():.2f}", f"{df_cpu[key].quantile(0.95):.2f}", f"3834"],
    [f"{keyword_off}", f"{df_off[key].mean():.2f}", f"{df_off[key].quantile(0.95):.2f}", f"7237"],
]
...
plt.savefig(f"dbml/{bench}{subject}_latency.pdf", format="pdf")
plt.show()